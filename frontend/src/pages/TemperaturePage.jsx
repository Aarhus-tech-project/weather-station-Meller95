...
export default function TemperaturePage() {
    ...
    const intervalQ = useQuery({
        queryKey: ["temp-range", fromIso, toIso],
        queryFn: () => getInterval(fromIso, toIso),
        // Auto-refetch kun hvis slut ligger ~nu
    ...
    // Numerisk tidsakse + 10 ticks
    const series = useMemo(
        () => data.map(d => ({ ...d, t: new Date(d.tidspunkt).getTime() })),
        [data]
    );
    ...
    return (
        <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
                <StatCard title="Seneste temperatur" value={latest.temperatur.toFixed(2)} unit={'\u00B0C'} />
                <StatCard title="Seneste fugtighed" value={latest.luftfugtighed.toFixed(2)} unit="%" />
                <StatCard title="Seneste tryk" value={latest.tryk.toFixed(2)} unit="hPa" />
            </div>
                    ...
                    <div style={{ marginTop: 30 }}>
                        ...
                        <ResponsiveContainer width="100%" height={320} >
                            ...
                            >
                                ...
                                <Line type="monotone" dataKey="temperatur" dot={false} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                </>
